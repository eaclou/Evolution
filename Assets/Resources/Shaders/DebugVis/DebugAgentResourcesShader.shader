		...
		Pass
		{
			...
			*/
			...
			v2f vert(uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				float camDist = distance(_WorldSpaceCameraPos, worldPosition);				
				
				float scale = camDist * 0.034; // * ((critterInitData.boundingBoxSize.x + critterInitData.boundingBoxSize.y) / 2.0) * lerp(critterInitData.spawnSizePercentage, 1, critterSimData.growthPercentage);
				quadPoint *= scale;
				quadPoint.y *= 0.5;

				worldPosition.z -= 0.05 * camDist;  // display above creature's head
			...
			{	
				...
				float3 growthBarColor = float3(1,1,1) * 0.4;
				finalColor.rgb = lerp(growthBarColor, float3(0,0,0), saturate((i.uv.x - i.critterResources.w) * 100));

				float energyDanger = saturate((1.0 - i.critterResources.y) - 0.5) * 2;

				if(i.uv.y > 0.25) {  // HEALTH
				...
				finalColor.rgb = lerp(finalColor.rgb, float3(1,0.25,0.05) * 0.263, energyDanger);
