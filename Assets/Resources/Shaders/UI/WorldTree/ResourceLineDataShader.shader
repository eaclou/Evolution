Shader "UI/WorldTree/ResourceLineDataShader"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			struct ResourceLineData {
				float3 worldPos;
				float4 color;

			};
			StructuredBuffer<float3> quadVerticesCBuffer;
			StructuredBuffer<ResourceLineData> resourceLineDataCBuffer;
			...
			v2f vert (uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				ResourceLineData dataPrev = resourceLineDataCBuffer[max(0, inst - 1)];
				ResourceLineData data = resourceLineDataCBuffer[inst];
				...
				//worldPosition = dataPrev.worldPos + quadData*0.04;
