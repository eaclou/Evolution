...
Shader "DataVis/StatsPanelGraphShader"
{
	Properties
	{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{	
				//float4 testColor = tex2D(_MainTex, IN.texcoord); // *** TEMP!***
				//testColor.rgb = testColor.rgb / _MaxValue;
				//testColor.rgb = float3(0.6, 0.1, 0.2);
				//testColor.a = 1;
				//return testColor;
				...
				float4 fitnessScores = tex2D(_MainTex, float2(finalCoords.x, 0)) / _MaxValue;
