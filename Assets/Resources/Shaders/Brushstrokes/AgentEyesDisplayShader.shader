Shader "Unlit/AgentEyesDisplayShader"
{
	Properties
	{
	...
	{		
		...
		Pass
		{
			...
			v2f vert(uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				float2 scale = float2(eyeData.localScale.x * aspect, eyeData.localScale.x * (1.0 / aspect)) * (1 + length(agentSimData.size * 0.15)) * saturate(agentSimData.maturity * 4 - 3);
				...
				float animFrame = sin(agentSimData.eatingStatus * 3.141592) * 0.99;	
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float4 finalColor = float4(rgb, brushColor.a * saturate(2.0 - agentSimData.decay * 2));
