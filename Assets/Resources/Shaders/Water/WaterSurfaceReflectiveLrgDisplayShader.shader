Shader "Water/WaterSurfaceReflectiveLrgDisplayShader"
{
	Properties
	{
	...
	{		
		...
		Pass
		{
			...
			v2f vert(uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				float2 scale = waterQuadData.localScale;
				//scale.x *= 0.76;
				//scale.y = *= 2.5; //scale.y * (1 + saturate(waterQuadData.speed * 64));
				...
				float reflectLerp = GetReflectionLerpLrg(worldPosition, surfaceNormal, viewDot, _CamDistNormalized * 0.25 + 0.2, _CamFocusPosition, vignetteRadius);
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				finalColor.a *= saturate(reflectLerp * 8) * 0.67;
