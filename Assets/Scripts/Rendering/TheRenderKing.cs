...
public class TheRenderKing : MonoBehaviour {
    ...
    */
    ...
    private ComputeBuffer critterPortraitStrokesCBuffer;
    ...
    *///$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  RENDER PIPELINE PSEUDOCODE!!!  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    private void InitializeCritterUberStrokesBuffer() {
        ...
        critterPortraitStrokesCBuffer = new ComputeBuffer(GetNumUberStrokesPerCritter(), GetMemorySizeCritterUberStrokeData());
    }
    ...
    public void UpdateCritterGenericStrokesData(Agent agent) { //int critterIndex, AgentGenome genome) {
        ...
        critterPortraitStrokesCBuffer.SetData(singleCritterGenericStrokesArray); 
        // SET DATA:::        
    ...
    private void GenerateCritterBodyBrushstrokes(ref CritterUberStrokeData[] strokesArray, CritterGenomeInterpretor.BrushPoint[] brushPointArray, Agent agent) {
    ...
    private void GenerateCritterEyeBrushstrokes(ref CritterUberStrokeData[] strokesArray, Agent agent) {
    ...
    private void GenerateCritterMouthTeethBrushstrokes(ref CritterUberStrokeData[] strokesArray, Agent agent) {
    ...
    private void AddUberBrushPoint(ref CritterUberStrokeData[] strokesArray, Agent agent, Vector3 initCoords, Vector2 uv, int brushIndex) {
        CritterGenomeInterpretor.BrushPoint newBrushPoint = new CritterGenomeInterpretor.BrushPoint();                
    ...
    private void GenerateCritterPectoralFinsBrushstrokes(ref CritterUberStrokeData[] strokesArray, Agent agent) {

    }
    private void GenerateCritterDorsalFinBrushstrokes(ref CritterUberStrokeData[] strokesArray, Agent agent) {
    ...
    private void GenerateCritterTailFinBrushstrokes(ref CritterUberStrokeData[] strokesArray, Agent agent) {
    ...
    private void GenerateCritterSkinDetailBrushstrokes(ref CritterUberStrokeData[] strokesArray, Agent agent) {
    ...
    private void SimCritterGenericStrokes() {
        ...
        int kernelCSSimulateCritterPortraitStrokes = computeShaderCritters.FindKernel("CSSimulateCritterGenericStrokes");        
        computeShaderCritters.SetTexture(kernelCSSimulateCritterPortraitStrokes, "velocityRead", fluidManager._VelocityA);
        computeShaderCritters.SetBuffer(kernelCSSimulateCritterPortraitStrokes, "critterInitDataCBuffer", treeOfLifePortraitCritterInitDataCBuffer);
        computeShaderCritters.SetBuffer(kernelCSSimulateCritterPortraitStrokes, "critterSimDataCBuffer", treeOfLifePortraitCritterSimDataCBuffer);
        computeShaderCritters.SetBuffer(kernelCSSimulateCritterPortraitStrokes, "critterGenericStrokesWriteCBuffer", critterPortraitStrokesCBuffer);
        computeShaderCritters.SetFloat("_MapSize", SimulationManager._MapSize);
        computeShaderCritters.Dispatch(kernelCSSimulateCritterPortraitStrokes, critterPortraitStrokesCBuffer.count / 16, 1, 1);
    }
    ...
    private void SimTreeOfLife() {
        ...
        float angle = Mathf.Cos(Time.realtimeSinceStartup * 0.37f);
        float angle2 = angle + (float)selectedSpeciesID; // + Time.realtimeSinceStartup * 0.1f; // + (Mathf.PI * 0.5f);
        Vector2 facingDir = new Vector2(Mathf.Cos(angle2 + (Mathf.PI * 0.5f)), Mathf.Sin(angle2 + (Mathf.PI * 0.5f)));
        simData.velocity = facingDir.normalized; //new Vector2(0f, 1f); // facingDir.normalized;
        ...
        simData.accel = (Mathf.Sin(Time.realtimeSinceStartup * 1.79f) * 0.5f + 0.5f) * 0.081f; // Mathf.Clamp01(simManager.agentsArray[i].curAccel) * 1f; // ** RE-FACTOR!!!!
		simData.smoothedThrottle = (Mathf.Sin(Time.realtimeSinceStartup * 2f + 0.4f) * 0.5f + 0.5f) * 0.8f;
        ...
        treeOfLifePortraitCritterSimDataCBuffer = new ComputeBuffer(1, sizeof(float) * 20);
        ...
        float isDraggingNode = 0f;
        if(simManager.uiManager.isDraggingSpeciesNode) {
            isDraggingNode = 1f;
        }
        computeShaderTreeOfLife.SetFloat("_IsDragging", isDraggingNode);
        computeShaderTreeOfLife.SetVector("_DraggingForceVector", new Vector4(simManager.uiManager.smoothedMouseVel.x, simManager.uiManager.smoothedMouseVel.y, 0f, 0f));        
    ...
    public void RenderSimulationCameras() { // **** revisit
        ...
        cmdBufferSpeciesPortraitDisplay.ClearRenderTarget(true, true, new Color(0f,0f,0f,1f), 1.0f);  // clear -- needed???
        ...
        treeOfLifeCritterPortraitMat.SetPass(0);
        treeOfLifeCritterPortraitMat.SetBuffer("quadVerticesCBuffer", quadVerticesCBuffer);
        treeOfLifeCritterPortraitMat.SetBuffer("critterInitDataCBuffer", treeOfLifePortraitCritterInitDataCBuffer);
        treeOfLifeCritterPortraitMat.SetBuffer("critterSimDataCBuffer", treeOfLifePortraitCritterSimDataCBuffer);
        treeOfLifeCritterPortraitMat.SetBuffer("critterGenericStrokesCBuffer", critterPortraitStrokesCBuffer);    
        treeOfLifeCritterPortraitMat.SetTexture("_WaterSurfaceTex", baronVonWater.waterSurfaceDataRT1);
        treeOfLifeCritterPortraitMat.SetFloat("_MapSize", SimulationManager._MapSize);          
        cmdBufferSpeciesPortraitDisplay.DrawProcedural(Matrix4x4.identity, treeOfLifeCritterPortraitMat, 0, MeshTopology.Triangles, 6, critterPortraitStrokesCBuffer.count);
    ...
    private void Render() {
        ...
        //baronVonTerrain.groundStrokesLrgDisplayMat.SetTexture("_NutrientTex", simManager.foodManager.nutrientMapRT1);
        ...
        //baronVonTerrain.groundStrokesMedDisplayMat.SetTexture("_NutrientTex", simManager.foodManager.nutrientMapRT1);
        cmdBufferTest.SetGlobalTexture("_RenderedSceneRT", renderedSceneID); // Copy the Contents of FrameBuffer into brushstroke material so it knows what color it should be
        ...
        //baronVonTerrain.groundStrokesSmlDisplayMat.SetTexture("_NutrientTex", simManager.foodManager.nutrientMapRT1);
        cmdBufferTest.SetGlobalTexture("_RenderedSceneRT", renderedSceneID); // Copy the Contents of FrameBuffer into brushstroke material so it knows what color it should be
        ...
        */
        ...
        critterUberStrokeShadowMat.SetTexture("_AltitudeTex", baronVonTerrain.terrainHeightMap);
        ...
        cmdBufferTest.SetGlobalTexture("_RenderedSceneRT", renderedSceneID);
        cmdBufferTest.DrawProcedural(Matrix4x4.identity, critterUberStrokeShadowMat, 0, MeshTopology.Triangles, 6, critterGenericStrokesCBuffer.count);
        ...
        */
        ...
        if(simManager.uiManager.treeOfLifePanelOn) {
            ...
            treeOfLifeStemSegmentsMat.SetInt("_DraggingID", simManager.uiManager.treeOfLifeManager.draggingID);
            treeOfLifeStemSegmentsMat.SetInt("_CurDeepestGraphDepth", simManager.masterGenomePool.currentHighestDepth);
            ...
            treeOfLifeBackdropPortraitBorderMat.SetPass(0);
            treeOfLifeBackdropPortraitBorderMat.SetBuffer("quadVerticesCBuffer", quadVerticesCBuffer);
            treeOfLifeBackdropPortraitBorderMat.SetBuffer("treeOfLifeBasicStrokeDataCBuffer", treeOfLifePortraitBorderDataCBuffer);
            treeOfLifeBackdropPortraitBorderMat.SetVector("_TopLeftCornerWorldPos", simManager.cameraManager.worldSpaceTopLeft);
            treeOfLifeBackdropPortraitBorderMat.SetVector("_CamRightDir", simManager.cameraManager.worldSpaceCameraRightDir);
            treeOfLifeBackdropPortraitBorderMat.SetVector("_CamUpDir", simManager.cameraManager.worldSpaceCameraUpDir);
            treeOfLifeBackdropPortraitBorderMat.SetFloat("_CamScale", simManager.uiManager.treeOfLifeManager.camScale);        
            ...
            treeOfLifePortraitMat.SetPass(0);
            treeOfLifePortraitMat.SetBuffer("quadVerticesCBuffer", quadVerticesCBuffer);
            treeOfLifePortraitMat.SetBuffer("critterInitDataCBuffer", treeOfLifePortraitCritterInitDataCBuffer);
            treeOfLifePortraitMat.SetBuffer("critterSimDataCBuffer", treeOfLifePortraitCritterSimDataCBuffer);
            ...
            // PORTRAIT EYES!!!!
            treeOfLifePortraitEyeMat.SetPass(0);
            treeOfLifePortraitEyeMat.SetBuffer("quadVerticesCBuffer", quadVerticesCBuffer);
            treeOfLifePortraitEyeMat.SetBuffer("critterInitDataCBuffer", treeOfLifePortraitCritterInitDataCBuffer);
            treeOfLifePortraitEyeMat.SetBuffer("critterSimDataCBuffer", treeOfLifePortraitCritterSimDataCBuffer);
            treeOfLifePortraitEyeMat.SetBuffer("agentEyesStrokesCBuffer", treeOfLifePortraitEyeDataCBuffer);     
            treeOfLifePortraitEyeMat.SetTexture("_AltitudeTex", baronVonTerrain.terrainHeightMap);
            treeOfLifePortraitEyeMat.SetTexture("_VelocityTex", fluidManager._VelocityA);
            treeOfLifePortraitEyeMat.SetTexture("_WaterSurfaceTex", baronVonWater.waterSurfaceDataRT1);
            treeOfLifePortraitEyeMat.SetFloat("_MapSize", SimulationManager._MapSize);
            treeOfLifePortraitEyeMat.SetVector("_TopLeftCornerWorldPos", simManager.cameraManager.worldSpaceTopLeft);
            treeOfLifePortraitEyeMat.SetVector("_CamRightDir", simManager.cameraManager.worldSpaceCameraRightDir);
            treeOfLifePortraitEyeMat.SetVector("_CamUpDir", simManager.cameraManager.worldSpaceCameraUpDir);
            treeOfLifePortraitEyeMat.SetFloat("_CamScale", simManager.uiManager.treeOfLifeManager.camScale);        
            treeOfLifePortraitEyeMat.SetFloat("_AnimatedScale1", 1f);
    ...
    private void OnDisable() {
        ...
        if(critterPortraitStrokesCBuffer != null) {
