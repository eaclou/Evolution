Shader "Unlit/FloatyBitsDisplayShader"
{
	...
	{		
		...
		Pass
		{
			...
			StructuredBuffer<float2> floatyBitsCBuffer;
			...
			v2f vert(uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				float3 worldPosition = float3(floatyBitsCBuffer[inst] * 140 - 70, -0.05);
				...
				float2 scale = float2(1, 1) * 0.1;
				quadPoint *= float3(scale, 1.0);

				// ROTATION:
				float rotationAngle = 0; //random1 * 10.0 * 3.141592;  // radians
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float4 finalColor = float4(0,0,0,0.75); //texColor * _Tint * float4(i.color, 1);
				return finalColor;
